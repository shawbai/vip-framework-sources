 ...
 * <li>applies the appropriate modification on rollback (actual rollback or
 * setting rollback-only);
 * <li>triggers registered synchronization callbacks (if transaction
 * synchronization is active).
 * </ul>
 *
 * <p>
 * Subclasses have to implement specific template methods for specific states of
 * a transaction, e.g.: begin, suspend, resume, commit, rollback. The most
 * important of them are abstract and must be provided by a concrete
 * implementation; for the rest, defaults are provided, so overriding is
 * optional.
 *
 * <p>
 * Transaction synchronization is a generic mechanism for registering callbacks
 * that get invoked at transaction completion time. This is mainly used
 * internally by the data access support classes for JDBC, Hibernate, JPA, etc
 * when running within a JTA transaction: They register resources that are
 * opened within the transaction for closing at transaction completion time,
 * allowing e.g. for reuse of the same Hibernate Session within the transaction.
 * The same mechanism can also be leveraged for custom synchronization needs in
 * an application.
 *
 * <p>
 * The state of this class is serializable, to allow for serializing the
 * transaction strategy along with proxies that carry a transaction interceptor.
 * It is up to subclasses if they wish to make their state to be serializable
 * too. They should implement the {@code java.io.Serializable} marker interface
 * in that case, and potentially a private {@code readObject()} method
 * (according to Java serialization rules) if they need to restore any transient
 * state.
 *
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	/**
	 * Always activate transaction synchronization, even for "empty"
	 * transactions that result from PROPAGATION_SUPPORTS with no existing
	 * backend transaction.
	 * 
	...
	/**
	 * Activate transaction synchronization only for actual transactions, that
	 * is, not for empty ones that result from PROPAGATION_SUPPORTS with no
	...
	/**
	 * Set the transaction synchronization by the name of the corresponding
	 * constant in this class, e.g. "SYNCHRONIZATION_ALWAYS".
	 * 
	 * @param constantName
	 ...
	 * <p>
	 * Note that transaction synchronization isn't supported for multiple
	 * concurrent transactions by different transaction managers. Only one
	 * transaction manager is allowed to activate it at any time.
	 * 
	 * @see #SYNCHRONIZATION_ALWAYS
	 ...
	 * <p>
	 * Typically initialized with an appropriate default by the concrete
	 * transaction manager subclass.
	 */
	...
	/**
	 * Set whether existing transactions should be validated before
	 * participating in them.
	 * <p>
	 * When participating in an existing transaction (e.g. with
	 * PROPAGATION_REQUIRED or PROPAGATION_SUPPORTS encountering an existing
	 * transaction), this outer transaction's characteristics will apply even to
	 * the inner transaction scope. Validation will detect incompatible
	 * isolation level and read-only settings on the inner transaction
	 * definition and reject participation accordingly through throwing a
	...
	/**
	 * Return whether existing transactions should be validated before
	 * participating in them.
	 * 
	 * @since 2.5.1
	 */
	 ...
	 * {@code PlatformTransactionManager.rollback()} call according to a
	 ...
	 * {@code setRollbackOnly} calls on a {@code TransactionStatus}, which will
	 * always cause an eventual global rollback (as it might not throw an
	 * exception after the rollback-only call).
	 * <p>
	...
	/**
	 * Set whether to fail early in case of the transaction being globally
	 ...
	 * default only cause UnexpectedRollbackException at the outermost
	 * transaction boundary. This allows, for example, to continue unit tests
	 * even after an operation failed and the transaction will never be
	 * completed. All transaction managers will only fail earlier if this flag
	 * has explicitly been set to "true".
	 * 
	 * @since 2.0
	...
	/**
	 * Return whether to fail early in case of the transaction being globally
	 * marked as rollback-only.
	 * 
	 * @since 2.0
	 */
	 ...
	 * {@code doCommit} call. Typically not necessary and thus to be avoided, as
	 * it can potentially override the commit exception with a subsequent
	 * rollback exception.
	 * <p>
	 * Default is "false".
	 * 
	 ...
	 * {@code doGetTransaction}, {@code isExistingTransaction} and
	 * {@code doBegin}.
	 * 
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 3.2、如果是这三种传播行为，则创建事务
			...
			try {
				...
				// 创建TransactionStatus对象，注意statues对象中持有了definition、transaction、suspendedResources
				// 挂起的事务
		...
		} else { // 3.3 其他三种 PROPAGATION_SUPPORTS PROPAGATION_NOT_SUPPORTED
					// PROPAGATION_NEVER 非事务方式运行，但会做可能的同步。
	...
	private TransactionStatus handleExistingTransaction(TransactionDefinition definition, Object transaction,
		...
		// 当前存在事务的处理逻辑
		// 1、如果新的事务定义的传播行为是PROPAGATION_NEVER 不能有事务，则抛异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}
		// 2、 如果新的事务定义的传播行为是PROPAGATION_NOT_SUPPORTED 不支持事务，则挂起事务，以无事务方式运行
		...
		// 3 、如果新的事务定义的传播行为是PROPAGATION_REQUIRES_NEW 需要一个新事务，挂起当前事务，开启一个新的事务
		...
		// 4 、如果新的事务定义的传播行为是PROPAGATION_NESTED 内嵌事务
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			// 4.1 如果事务管理器实现不支持内嵌事务，则抛出异常
			...
			// 4.2 如果事务管理器实现支持保存点方式来进行内嵌事务的管理，则在事务上增加保存点
			...
			// 4.3 否则，则以内嵌事务的方式运行：内嵌 begin、commit/rollback
			else {
		...
		// 5 、如果新的事务定义的传播行为是PROPAGATION_SUPPORTS or PROPAGATION_REQUIRED
		// 在当前事务中运行
		// Assumably PROPAGATION_SUPPORTS or PROPAGATION_REQUIRED.
		if (debugEnabled) {
			logger.debug("Participating in existing transaction");
		}
		// 5.1、是否要对已存在的事务进行检查，如要则进行下面的检查
	...
	/**
	 * Determine the actual timeout to use for the given definition. Will fall
	 * back to this manager's default timeout if the transaction definition
	 * doesn't specify a non-default value.
	 * 
	 * @param definition
	...
	/**
	 * Suspend the given transaction. Suspends transaction synchronization
	 * first, then delegates to the {@code doSuspend} template method.
	 * 
	 * @param transaction
	 *            the current transaction object (or {@code null} to just
	 *            suspend active synchronizations, if any)
	 * @return an object that holds suspended resources (or {@code null} if
	 ...
	 *            {@code suspend} (or {@code null} to just resume
	...
	/**
	 * Reactivate transaction synchronization for the current thread and resume
	 * all given synchronizations.
	 * 
	 ...
	 * transactions and programmatic rollback requests. Delegates to
	 * {@code isRollbackOnly}, {@code doCommit} and {@code rollback}.
	 * 
	...
	/**
	 * Process an actual commit. Rollback-only flags have already been checked
	 * and applied.
	 * 
	 * @param status
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				// 1、为提交做准备（事务管理器实现子类有提交准备要做则扩展这个方法）
				prepareForCommit(status);
				// 2、触发提交前的监听回调，这是做事务同步控制需要用到
				triggerBeforeCommit(status);
				// 3、触发完成前的监听回调，这是做事务同步控制需要用到
				triggerBeforeCompletion(status);
				beforeCompletionInvoked = true;

				// 4、如果当前执行提交的是个保存点，则释放保存点
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				// 1、触发完成前的监听回调，这是做事务同步控制需要用到
				triggerBeforeCompletion(status);
				// 2、如果当前执行回滚的事务是一个保存点，则回滚到保存点
				if (status.hasSavepoint()) {
					if (status.isDebug()) {
				...
				// 3、如果当前执行回滚的事务是一个新事务，则执行回滚操作
				else if (status.isNewTransaction()) {
					if (status.isDebug()) {
						logger.debug("Initiating transaction rollback");
					}
					doRollback(status); // 子类实现执行具体的回滚逻辑
				}
				// 4、其他事务情况处理
				else {
					// 4.1 大事务的加入者 Participating in larger transaction
					if (status.hasTransaction()) {
						// 如果是本地回滚 或 参与者失败则全局回滚，则设置仅回滚
					...
					// Unexpected rollback only matters here if we're asked to
					// fail early
			...
			// Raise UnexpectedRollbackException if we had a global
	 ...
	 * <p>
	 * To be called by this abstract manager itself, or by special
	 ...
	 * @see #registerAfterCompletionWithExistingTransaction(Object,
	 *      java.util.List)
	 ...
	 * Consequently, a {@code doGetTransaction} implementation will usually look
	 * for an existing transaction and store corresponding state in the returned
	 * transaction object.
	 * 
	 ...
	 * <p>
	 * Default is {@code true}, which causes delegation to
	 * DefaultTransactionStatus for creating and holding a savepoint. If the
	 * transaction object does not implement the SavepointManager interface, a
	 ...
	 * call to {@code doBegin} - within the context of an already existing
	 * transaction. The {@code doBegin} implementation needs to handle this
	 * accordingly in such a scenario. This is appropriate for JTA, for example.
	 * 
	 ...
	 * will be called to start a nested transaction when necessary. In such a
	 * context, there will be an active transaction: The implementation of this
	 * method has to detect this and start an appropriate nested transaction.
	 * 
	 * @param transaction
	...
	/**
	 * Suspend the resources of the current transaction. Transaction
	 * synchronization will already have been suspended.
	 * <p>
	 ...
	 * @param transaction
	 *            transaction object returned by {@code doGetTransaction}
	 * @return an object that holds suspended resources (will be kept unexamined
	 *         for passing it into doResume)
	 ...
	 * rollback-only marker in the transaction itself, therefore they can't
	 * handle rollback-only transactions as part of transaction commit. Hence,
	 ...
	 * <p>
	 * Override this to return "true" if the concrete transaction manager
	 * expects a {@code doCommit} call even for a rollback-only transaction,
	 * allowing for special handling there. This will, for example, be the case
	 * for JTA, where {@code UserTransaction.commit} will check the read-only
	 ...
	 * <p>
	 * If this method returns "true" but the {@code doCommit} implementation
	 * does not throw an exception, this transaction manager will throw an
	 * UnexpectedRollbackException itself. This should not be the typical case;
	 * it is mainly checked to cover misbehaving JTA providers that silently
	 * roll back even when the rollback has not been requested by the calling
	 * code.
	 * 
	 * @see #doCommit
	 ...
	 * completed yet. This is for example the case when participating in an
	 * existing JTA or EJB CMT transaction.
	 * <p>
	 * The default implementation simply invokes the {@code afterCompletion}
	 * methods immediately, passing in "STATUS_UNKNOWN". This is the best we can
	 * do if there's no chance to determine the actual outcome of the outer
	 * transaction.
	 * 
	 * @param transaction
	 *            transaction object returned by {@code doGetTransaction}
